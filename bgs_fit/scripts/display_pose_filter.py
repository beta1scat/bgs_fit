...
for row in range(num_pcd):
    for col in range(num_pcd):
        ...
        for i in range(3):
            cube_pcd = generateCubePcd(*cube_size, 5000)
            ...
            ellipsoid_pcd = generateEllipsoidPcd(*ellipsoid_size, 5000)
            ...
            cone_poses_center = gen_cone_center_pick_poses(cone_size[2], 8)
            cone_poses = cone_poses_side + cone_poses_center
            for pose in cone_poses:
                poses.append(T_cone*pose)

            ellipsoid_poses_center = gen_ellipsoid_center_pick_poses(8)
...
coord_ref = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.2, origin=[0, 0, 0])
coord_ref.transform(SE3([4, 4, 4])*ref_pose)
...
o3d.visualization.draw_geometries([*pcds, *posesMeshAxisFiltered, *binMesh, coord_ori, coord_ref])

gripper = Gripper()
# gripper.transform(SE3([0,0,1]))
coord_ori = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.8, origin=[0, 0, 0])
coord_tcp = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.8, origin=[0, 0, 0])
coord_tcp.transform(gripper.get_tcp_in_flange())
o3d.visualization.draw_geometries([*gripper.get_meshes(), coord_ori, coord_tcp])

for pose in posesFiltered:
    T = pose*gripper.get_tcp_in_flange().inv()
    gripper.transform(T)
    o3d.visualization.draw_geometries([*pcds, *posesMeshAxisFiltered, *binMesh, coord_ori, coord_ref, *gripper.get_meshes()])

...
def show_anns(anns):
    ...
    for idx in range(numAnn):
        ...
        img[m] = np.array(convert_rgb_to_255(color_mask), int)
        ...
        ax.text(bbox[0] + (bbox[2] / 2) - 15, bbox[1] + (bbox[3] / 2) - 15, f"{idx}", size = 15)
...
while True:
    ...
    typeList = []
    num_class = 3
    for maskDict in masks:
        # 选择分割结果，并展示
        # mask = masks[0]
        mask = maskDict['segmentation']
        ...
        print(pred)
        print(pred_choice)
        typeList.append(pred_choice.item())
        pcdIdx += 1
    print(typeList)
    np.savetxt(os.path.join(data_path, 'class.txt'), np.array(typeList).astype(int), delimiter=',')